...
import type { ExperimentRun, GenerationRun, AllocationSnapshot, GenerationReport } from "@/platform/schema/experiment";
import type { Persona } from "@/platform/schema/persona";
import { getCalibratedPersonaSet } from "@/domains/calibration/store";
import { readPage, type PersonaReading } from "@/domains/sim/reading";
...
import { mapPool } from "@/platform/async/pool";
import { buildComputedReport } from "./computed-report";
import type { ExperimentProgressReporter } from "@/domains/loop/experiment-progress";
import { loadSourceBaselineHtml } from "@/domains/deploy/write-html";
import type { VariantMetrics } from "@/platform/schema/events";
...
export async function runExperiment(cfg: RunConfig = DEFAULT_CONFIG): Promise<ExperimentRun> {
  ...
  const { GENERATION_0: gen0 } = await import("@/content/variants");
