...
import { CONVERSION_EVENT_NAMES } from "@/domains/analytics/posthog-events";
...
export async function fetchPostHogMetrics(windowDays = 30): Promise<RealMetricsSnapshot | null> {
  ...
  const conversionEvents = CONVERSION_EVENT_NAMES.map((e) => `'${e}'`).join(", ");

  const query = `
    SELECT
      properties.variant_id AS variant_id,
      count(DISTINCT person_id) AS visitors,
      countIf(event IN (${conversionEvents})) AS cta_clicks,
      avgIf(toFloat(coalesce(properties.percent_scrolled, properties.scroll_depth_pct)), event = 'scroll_depth') / 100 AS avg_scroll,
      countIf(event = '$pageleave' AND toFloat(properties.max_scroll_depth) < 0.15) AS bounces,
      countIf(event = '$pageview' OR event = '$pageleave') AS sessions
    FROM events
