        ...
        <section key={gen.generation} className="mb-12">
          ...
          <div className="mb-6 overflow-x-auto rounded-2xl border border-slate-200">
            <table className="w-full text-left text-sm">
              <thead className="bg-schole-surface text-xs uppercase tracking-wide text-slate-500">
                <tr>
              ...
              <tbody>
                {gen.metrics.map((m, i) => {
                  ...
                  return (
                    <tr key={m.variantId} className="border-t border-slate-200">
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`font-bold ${i === 0 ? "text-emerald-400" : "text-slate-700"}`}
                        >
                          {m.fitness.toFixed(1)}
                        </span>
                      </td>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <div className="rounded-2xl border border-slate-200 bg-white p-5">
              <h3 className="font-semibold text-slate-900">Bandit traffic allocation</h3>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5">
              <h3 className="font-semibold text-slate-900">Top conversion blockers</h3>
              <p className="mt-1 text-xs text-slate-500">
          ...
          <div className="mt-6">
            <h3 className="mb-3 font-semibold text-slate-900">Evaluator scorecards</h3>
            <div className="grid gap-4 md:grid-cols-2">
              {gen.report.scorecards.map((sc) => (
                ...
                >
                  ...
                  <p className="mt-2 text-sm text-slate-700">{sc.summary}</p>
          ...
          <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-5">
            <h3 className="font-semibold text-slate-900">Generation insights</h3>
...
function Score({ label, value }: { label: string; value: number }) {
  return (
    <div>
      ...
      <div className="font-semibold text-slate-900">{value.toFixed(1)}/10</div>
    </div>
  );
}
...
function PBestBar({ value }: { value: number }) {
  const pct = value * 100;
  const color =
    pct >= 95 ? "bg-emerald-500" : pct >= 70 ? "bg-schole-primary" : "bg-slate-300";
  return (
...
function DecisionChip({ decision }: { decision: VariantDecision }) {
  const styles: Record<VariantDecision["status"], string> = {
    ...
    collecting: "bg-slate-200 text-slate-400",
  };
...
function DecisionNotes({ decisions }: { decisions: VariantDecision[] }) {
  ...
  if (!notable.length) {
    return (
      <p className="mb-6 rounded-xl border border-slate-200 bg-schole-surface px-4 py-3 text-xs text-slate-500">
    ...
    <section className="mb-10 grid gap-4 lg:grid-cols-2">
      <div className="rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <ul className="mt-3 space-y-2 text-xs leading-relaxed text-slate-400">
          <li>
            <b className="text-slate-700">Bayesian, not t-tests.</b> Thompson-sampled traffic
          ...
          <li>
            <b className="text-slate-700">Promote</b> when P(best) ≥ 95% <i>and</i> expected loss
            ≤ 0.1pp of conversion. <b className="text-slate-700">Kill</b> when P(beat baseline)
            &lt; 5%. Otherwise: keep collecting.
          </li>
          <li>
            <b className="text-slate-700">Guardrail:</b> a conversion winner is blocked from
            promotion if its bounce rate exceeds baseline by &gt;10% relative.
          </li>
          <li>
            <b className="text-slate-700">Winner&apos;s-curse control:</b> a Beta(3, 97) prior
      ...
      >
        ...
        {aaTest ? (
          ...
          <p className="mt-3 text-xs leading-relaxed text-slate-500">
            Not run yet. <code className="rounded bg-slate-100 px-1">npm run aa-test</code> runs
...
function FitnessCurve({
  ...
  return (
    <section className="mt-10 rounded-2xl border border-schole-primary/30 bg-schole-primary/5 p-6">
      <h2 className="font-semibold text-slate-900">Fitness over generations</h2>
      <div className="mt-4 flex items-end gap-4">
        {bestPerGen.map((m, i) => (
          <div key={i} className="flex flex-col items-center">
            <div
              className="w-12 rounded-t bg-schole-primary"
              style={{ height: `${Math.max(20, m.fitness * 2)}px` }}
            />
            <div className="mt-2 text-xs text-slate-500">Gen {i}</div>
            <div className="text-xs font-bold text-slate-900">{m.fitness.toFixed(0)}</div>
          </div>
        ))}
      </div>
      {gen0Best && lastBest && (
        <p className="mt-4 text-sm text-slate-400">
          ...
          <b className="text-slate-900">{gen0Best.fitness.toFixed(1)}</b> (gen 0,{" "}
          {variants.find((v) => v.id === gen0Best.variantId)?.name}) to{" "}
          <b className="text-slate-900">{lastBest.fitness.toFixed(1)}</b> (gen {run.generations.length - 1},{" "}
